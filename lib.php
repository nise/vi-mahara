...
class PluginBlocktypeExternalvideo extends MaharaCoreBlocktype {
    ...
    // xxx added
    public static function html5_code($url, $width, $height) {
        $width = (int) $width;
        $height = (int) $height;
        $url = hsc($url);
        return '<video width="' . $width . '" height="' . $height . '">'
            ...
            . '<embed src="' . $url . '" '
            . 'type="application/x-shockwave-flash" wmode="transparent" width="' . $width . '" '
            . 'height="' . $height . '" allowscriptaccess="never"></embed></video>';
    }
    ...
    public static function render_instance(BlockInstance $instance, $editing=false) {
        ...
        if (!isset($configdata['html'])) {
            ...
            else if (isset($configdata['type']) && $configdata['type'] == 'html5') {
                $configdata['html'] = $configdata['videoid'] = self::html5_code($url, $width, $height);
                unset($configdata['type']);
            }
        ...
        foreach ($sources as $name => $source) {
            if ($result = $source->process_url($url, $width, $height)) {
                if ($result['type'] == 'embed') {
                    ...
                    $result['html'] = self::html5_code($result['videoid'], $result['width'], $result['height']);
                }
                else {
